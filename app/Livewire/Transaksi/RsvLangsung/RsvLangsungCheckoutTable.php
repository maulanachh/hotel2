...
final class RsvLangsungCheckoutTable extends PowerGridComponent
{
    use WithExport;
    public string $primaryKey = 'tr_bill_total.billtotal_id';

    public string $sortField = 'tr_bill_total.billtotal_id';
    ...
    public function datasource(): Builder
    {
        return rsvLangsung::query()
            ...
            ->join('tr_bill_total', 'tr_reservasi_kmr.rsv_id', '=', 'tr_bill_total.rsv_id')
            ->where('tr_reservasi_kmr.sttsrsv_id', 4)
            ->select([
                'tr_bill_total.billtotal_id',
            ...
            ->orderBy('tr_reservasi_kmr.rsv_id', 'desc');
    }
    ...
    #[\Livewire\Attributes\On('cetak_invoice')]
    public function cetak_invoice($rowId): void
    {
        $url = "/transaksi/reservasi/direct/invoice/{$rowId}";
        $this->js("window.open('$url', '_blank')");
    }
    #[\Livewire\Attributes\On('cetak_invoice_dasar')]
    public function cetak_invoice_dasar($rowId): void
    {
        $this->js('alert( "invoice dasar")');
    }

    public function actions(rsvLangsung $row): array
    {
        return [
            Button::add('cetak_invoice')
                ->slot('<i class="bx bx bx-printer"></i> cetak invoice')
                ->id()
                ->class('btn btn-primary waves-effect waves-light')
                ->dispatch('cetak_invoice', ['rowId' => $row->billtotal_id]),
            // Button::add('cetak_invoice_dasar')
            //     ->slot('<i class="bx bx bx-printer"></i> cetak invoice dasar')
            //     ->id()
            //     ->class('btn btn-success waves-effect waves-light')
            //     ->dispatch('cetak_invoice_dasar', ['rowId' => $row->billtotal_id]),
        ];
    }
